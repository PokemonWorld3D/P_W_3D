...
#region Pokemon Canvas
	...
	void Update()
	{
		...
		if(targetPokemonPortrait.activeInHierarchy == true)
			HandleTargetGUI();
	}

	public void Save()
	{
		owner.GetComponent<_PlayerCharacter>().Save();
	}
	public void Quit()
	{
		owner.GetComponent<_PlayerCharacter>().Quit();
	}
	public void SetActivePokemon(_Pokemon playersActivePokemon)
	{
		activePokemon = playersActivePokemon;
		thisPokemon = owner.GetComponent<_PlayerCharacter>().activePokemon;
	...
	public void SetTarget(_Pokemon playersTarget)
	{
	...
	public void HandlePlayerPokemonGUI()
	{
		...
		float expFillAmount = ((float)currentEXP / (float)requiredEXP);
	...
	{
		string pokemonsName = target.pokemonName;
		if(target.nickName != "")
		{
			pokemonsName = activePokemon.nickName;
		}
		targetInfo.text = "Level " + target.level + " " + pokemonsName;
		int current_hp = target.curHP;
		int current_max_hp = target.curMaxHP;
		targetAvatar.sprite = target.avatar;
