...
public class _CalculateXP {

	public int CalculateCurrentXP(int level, _Pokemon.LevelingRates lvlrate){
		if(lvlrate == _Pokemon.LevelingRates.ERRATIC){
			return CalculateErraticCurrentXP(level);
		}
		if(lvlrate == _Pokemon.LevelingRates.FAST){
			return CalculateFastCurrentXP(level);
		}
		if(lvlrate == _Pokemon.LevelingRates.FLUCTUATING){
			return CalculateFluctuatingCurrentXP(level);
		}
		if(lvlrate == _Pokemon.LevelingRates.MEDIUM_FAST){
			return CalculateMediumFastCurrentXP(level);
		}
		if(lvlrate == _Pokemon.LevelingRates.MEDIUM_SLOW){
			return CalculateMediumSlowCurrentXP(level);
		}
		if(lvlrate == _Pokemon.LevelingRates.SLOW){
			return CalculateSlowCurrentXP(level);
		}
		return 0;
	}

	public int CalculateRequiredXP(int level, _Pokemon.LevelingRates lvlrate){
		if(lvlrate == _Pokemon.LevelingRates.ERRATIC){
